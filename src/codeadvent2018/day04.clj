...
(defn parse-record [r]
  (let [[year day month hour minute info]
    ...
    {:date (mapv #(Integer/parseInt %) [year day month])
     ...
     :wakeup (= info "up")
     :asleep (= info "asleep")
     :id (if (= (first info) \#) (Integer/parseInt (subs info 1)))}))


(defn agg-sleep-times [inp]
  (loop [input inp acum {} id (:id (first input))]
    (if (empty? input)
      acum
      (let [r (parse-record (first input))
            new-input (rest input)
            new-id (or (get r :id) id)]
        (recur new-input
               (assoc-in acum
                         [new-id (:time r)]
                         (inc (get-in acum [new-id (:time r)] 0)))
               new-id)))))

(defn part1 [input]
  (let [agg-times (agg-sleep-times input)
        [id [[_ min] _]] (apply max-key (comp second second)
                                (mapcat (fn [[id times]]
                                          (mapv #(identity [id %]) times))
                                        agg-times))]
    (println agg-times)
    (println [id min])
    (* id min)
